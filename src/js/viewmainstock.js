// API Configuration
        ...
        const SHEET_RANGE = "Main Stock!A2:F"; // Product ID(A), Product Name(B), Generic Name(C), Purchase QTY(D), Sell QTY(E), Remaining QTY(F)
        ...
        /**
         * NEW: Calculates the total remaining quantity for the displayed data.
         * @param {Array<Array<string>>} data The filtered stock data.
         * @returns {number} The total remaining quantity.
         */
        function calculateTotalRemainingStock(data) {
            return data.reduce((total, row) => {
                // row[5] is Remaining QTY (Column F)
                const quantity = parseQty(row[5]); 
        ...
        // Function to render the fetched/filtered data (MODIFIED)
        function renderData(data) {
            const dataContainer = document.getElementById('data-container');
            const totalQuantityValueSpan = document.getElementById('total-remaining-stock-value'); // NEW ID
            
            // NEW: Calculate and display total remaining stock
            const totalQuantity = calculateTotalRemainingStock(data);
