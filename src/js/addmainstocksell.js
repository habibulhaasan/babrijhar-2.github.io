...
function initializeRows() {
    ...
    // Clear existing rows except for the first HTML-defined row
    ...
    productData.forEach((product, index) => {
        ...
        row.insertCell(4).innerHTML = `<input type="number" name="sell_quantity[]" required value="0" oninput="calculateTotal(this)">`;
...
function addRow() {
    ...
    row.insertCell(1).innerHTML = `
        <input type="text" name="product_name[]" id="productInput1" oninput="showProductSuggestions(this)" autocomplete="off" required>
                            <div id="productSuggestions1" class="suggestions-box">`;
...
// Add 20 new rows to the Sell table
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("SellForm").addEventListener("submit", function(event) {
        ...
        //date picker
    const selectedDate = document.getElementById("sell_date").value;
    formData.append("sell_date", selectedDate);
        const rows = Array.from(document.querySelectorAll("#Sell tr")).slice(1); // Skip header row
        rows.forEach(row => {
            const quantity = parseFloat(row.querySelector('input[name="sell_quantity[]"]').value) || 0;
            if (quantity > 0) {
                ...
                formData.append("sell_price[]", row.querySelector('input[name="sell_price[]"]').value);
