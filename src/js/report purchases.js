...
function renderDateList(data) {
    if (!data || data.length === 0) {
        sellDataContainer.innerHTML = '<p class="info-message">No Purhcase data found in the sheet.</p>';
        return;
    }
    ...
    if (uniqueDates.length === 0) {
        sellDataContainer.innerHTML = '<p class="info-message">No valid dates found in the Purhcase data.</p>';
        return;
    }
    ...
    let html = '<h2>Select a Purhcase Date</h2><ul class="date-list">';
...
function displaySalesForDate(dateString) {
    ...
    let html = `
        ...
        <h2>Purhcase Records for ${displayDate}</h2>
...
async function fetchSellData() {
    ...
    allSellData = [];
    sellDataContainer.innerHTML = '<div class="loading-message"><i class="fa-solid fa-spinner fa-spin"></i> Loading Purhcase data...</div>';
